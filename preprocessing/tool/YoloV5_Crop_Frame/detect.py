...
def get_frame_count(video_path):
    ...
    if not cap.isOpened():
        return 0  # Safety check in case the video cannot be opened
    # Get total number of frames
    frame_count = int(cap.get(cv2.CAP_PROP_FRAME_COUNT))
    # Release video capture
    cap.release()
    return max(frame_count, 0)  # Ensure frame count is non-negative


def select_random_frames(total_frames, num_frames=100):
    if num_frames <= 0:
        return []  # Return an empty list if a non-positive number of frames is requested
    if total_frames <= num_frames:
        return list(range(total_frames))  # Return all frame indices if fewer frames are available than requested
    # Select num_frames random frame indices
    return random.sample(range(total_frames), num_frames)

# Load YOLOv5 model
device = select_device('')
model_yolo = attempt_load('/Users/lap01743/Downloads/WorkSpace/capstone_wed/XMT_Model/preprocessing/tool/YoloV5_Crop_Frame/face_detection_yolov5s.pt')
stride = int(model_yolo.stride.max())

# Input and output directories
input_folder = r'/Users/lap01743/Downloads/WorkSpace/capstone_wed/data_FF++_real_c23/original_sequences/actors/c23/videos'
output_folder = r'/Users/lap01743/Downloads/WorkSpace/capstone_wed/20k_FF++_actors_real_news/'
